...
const VALID_GRADES_STRING = ['DRP', 'INC', 'DFG', 'S', 'U', 'P'];
...
class EditStudentRecord extends React.Component {
        ...
        if(sectionId === this.TABLE){
            ...
            this.setState(prevState => ({
                ...
                if(typeof fresh === 'boolean' && tableBody.children.length === 1){
                    tableBody.children[0].addEventListener('input', this.inputEventListenerForCourseCode);
                    return;
                }
    ...
    inputEventListenerForCourseCode = (event) => {
        ...
        if(SPECIAL_COURSE_CODE.includes(element.innerHTML) && SPECIAL_COURSE_SWITCH){
            ...
            element.parentNode.children[3].innerHTML = isNaN(this.state.edit[0][1])? 0
                                                        ...
                                                        ?parseFloat(insideParenthesisReverse.exec(this.state.edit[0][2])) * parseFloat(this.state.edit[0][1])
                                                        : 'err';
                    ...
                    for(let i = 0; i < tableRow.children.length; i++){
                        ...
                        if(tableRow.children[i].classList.contains('weight')){
                            tableRow.children[i].innerHTML = isNaN(tableRow.children[i - 2].innerHTML)? 0
                                                            ...
                                                            ?parseFloat(insideParenthesisReverse.exec(tableRow.children[i - 1].innerHTML)[1]) * parseFloat(tableRow.children[i - 2].innerHTML)
                                                            : 'err';
                            continue;
                        }
            ...
            submitButton.onclick = (event) => {
                ...
                for(let i = 0; i < tableRow.children.length; i++){
                    if(tableRow.children[i].classList.contains('courseCode')){
                        if(/*courseCode_regex.exec(tableRow.children[i].innerHTML) != null &&*/ tableRow.children[i].innerHTML.length <= 20){
                            continue;
                        }
                        else{
    ...
    render() {
        return (
            <div hidden = {this.props.hide && !this.state.isEditOn} id = {this.props.id + '_' + this.state.progress} className={styles.container}>
                <button 
                    onClick={this.edit}
                ...
                </button>
                {
                    ...
                    <div className='position-absolute'>
                        <Toast bg="warning" onClose={() => this.setState({ isInvalid1: false })} show={this.state.isInvalid1} position="top-center" delay={3000} autohide>
                            <Toast.Body>The entered course code is invalid.</Toast.Body>
                        </Toast>
                    </div>
                }
                {
                    this.state.isInvalid2 &&
                    <div className='position-absolute'>
                        <Toast bg="warning" onClose={() => this.setState({ isInvalid2: false })} show={this.state.isInvalid2} position="top-center" delay={3000} autohide>
                            <Toast.Body>The entered units is invalid.</Toast.Body>
                        </Toast>
                    </div>
                }
                {
                    ...
                    <div className='position-absolute'>
                        <Toast bg="warning" onClose={() => this.setState({ isInvalid3: false })} show={this.state.isInvalid3} position="top-center" delay={3000} autohide>
                            <Toast.Body>The entered units is invalid.</Toast.Body>
                        </Toast>
                    </div>
                }
                {
                    this.state.isInvalid4 &&
                    <div className='position-absolute'>
                        <Toast bg="warning" onClose={() => this.setState({ isInvalid4: false })} show={this.state.isInvalid4} position="top-center" delay={3000} autohide>
                            <Toast.Body>The entered grade is invalid.</Toast.Body>
                        </Toast>
                    </div>
                }
            </div>
